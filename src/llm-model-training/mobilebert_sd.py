...
num_class_0_train = 141  # number of samples for class 0 (False/Not-CW) in training set
num_class_1_train = 343  # number of samples for class 1 (True/CW) in training set
...
def compute_metrics(p: EvalPrediction):
    ...
    # preds = (probs[:, 1] > 0.4).int()  # Adjust threshold here
    # labels = p.label_ids
...
eps.reverse()
# eps = [219, 555, 562, 563, 564, 565, 566, 629, 630, 672]
...
for i, ep_id in enumerate(eps):
    ...
    filename = f"sd_ground_truth_ep_{ep_id}.csv"
...
# Apply label mapping
dataset = dataset.map(map_labels, batched=True).remove_columns(["article_url"]).rename_column("stance", "label")
...
tokenizer = AutoTokenizer.from_pretrained("google/mobilebert-uncased", do_lower_case=True)
auto_model = AutoModelForSequenceClassification.from_pretrained("google/mobilebert-uncased", num_labels=2)

print("------------------------------------------------------------------------------------------")
print(
    f"Pre-Trained Model 'google/mobilebert-uncased' has been Loaded: \
...
# print("****USING FOCAL LOSS METHOD****")
trainer.train()

model_path = os.path.join(current_dir, "saved-models", "mobilebert-sd-custom")
