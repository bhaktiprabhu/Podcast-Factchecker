...
class CustomTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        logits = outputs.get("logits")
        # compute custom loss
        loss_fct = torch.nn.CrossEntropyLoss(weight=class_weights_train.to(logits.device))
...
print("------------------------------------------------------------------------------------------")
print(
    f"Pre-Trained Model 'albert-base-v2' (trained on {sum(p.numel() for p in auto_model.parameters() if p.requires_grad)} parameters) has been Loaded."
)
print("------------------------------------------------------------------------------------------")
...
model_path = os.path.join(current_dir, "saved-models", "albert-focal")
