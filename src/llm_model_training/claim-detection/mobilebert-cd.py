"""Training of MobileBERT Transformer model with Podcast Claim Detection Dataset"""
...
print("------------------------------------------------------------------------------------------")
print(
    f"Pre-Trained Model 'mobilebert-uncased' (trained on {sum(p.numel() for p in auto_model.parameters() if p.requires_grad)} parameters) has been Loaded."
)
print("------------------------------------------------------------------------------------------")
...
model_path = os.path.join(current_dir, "saved-models", "mobilebert-podcast-claim-detection")
